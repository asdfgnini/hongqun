...
{	
	...
	sprintf(ClientID,"%x",(unsigned int)temp);  //构建客户端ID，并存入缓冲区
	ClientID_len = strlen(ClientID);                                      //计算客户端ID的长度
	
	memset(Username,0,128);                                               //用户名的缓冲区全部清零
	sprintf(Username,"%x",(unsigned int)temp);                      //构建用户名，并存入缓冲区
	...
	sprintf(ServerIP,"%s","47.108.144.41");                  //构建服务器域名
...
void MQTT_DealPushdata_Qs0(unsigned char *redata)
{
	...
	do{
		...
		do{
			...
			if (g_timeJiffies > 4) { //  || pack_num > R_NUM
				u1_printf("处理服务器发来的等级0的推送失败0\r\n");  // 莫名的处理服务器发来的等级0的推送 失败
				return;
			}
		}while ((temp & 128) != 0);
		...
		temp_len = 0;
		if (g_timeJiffies > 4) { //  || pack_num > R_NUM
			u1_printf("处理服务器发来的等级0的推送失败1\r\n");  // 莫名的处理服务器发来的等级0的推送 失败
			return;
		}
	}while(re_len!=0);                                  			//如果接收的数据总长度等于0了，说明统计完毕了
	u1_printf("-->收到%d个服务器推送数据\r\n",pack_num);//串口输出信息
	...
	for(i=0;i<pack_num;i++){                                        //已经统计到了接收的推送个数，开始for循环，取出每个推送的数据 		
		do{
			...
			local++;
			if (g_timeJiffies > 4) { //  || pack_num > R_NUM
				u1_printf("处理服务器发来的等级0的推送失败2\r\n");  // 莫名的处理服务器发来的等级0的推送 失败
				return;
			}
		}while ((temp & 128) != 0);
		u1_printf("-->完成%d个服务器推送\r\n",pack_num);//串口输出信息
